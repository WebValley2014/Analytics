...
ranking_w.writerow(["FEATURE_ID", "FEATURE_NAME", "MEAN_POS", "MEDIAN_ALL", "MEDIAN_0", "MEDIAN_1", "FOLD_CHANGE", "LOG2_FOLD_CHANGE", "STD_DEV_0", "STD_DEV_1"])
...
for i, pos in zip(BORDA_ID, BORDA_POS):
    ...
    std_c = np.zeros(np.shape(classes)[0]) #added by kevin, calculate standard deviations for error bars
    for jj,c in enumerate(classes):
        med_c[jj] = np.median(x[y==c, i])
        std_c[jj] = np.std(x[y==c, i])
    with np.errstate(divide='ignore'):
        fc = med_c[1] / med_c[0]
    log2fc = np.log2(fc)
    ranking_w.writerow([ i, var_names[i], pos+1, med_all, med_c[0], med_c[1], fc, log2fc, std_c[0], std_c[1]])
rankingf.close()
