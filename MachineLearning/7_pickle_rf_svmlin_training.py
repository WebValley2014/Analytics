...
parser.add_argument('ML_TYPE', type=str, choices=['randomForest', 'l2r_l2loss_svc', 'l2r_l2loss_svc_dual', 'l2r_l1loss_svc_dual', 'l2r_lr_dual', 'l1r_l2loss_svc'], help='RandomForest or SVM type')
...
#parser.add_argument('CHECK_VAR_ZEROS', type=np.int, default=1, help='If 1 (default), repeats splitting of data if test set contains features with variance of zero to prevent calculation errors. Set to 0 if algorithm gets stuck to disable zero check')
...
parser.add_argument('--max_zero_checks', type=np.int, default=25, help='Maximum number of times the algorithm tries to split the data in each CV so that the training set has no feature with a variance of zero to avoid calculation errors (zero division). After having tried this number of times, the algorithm will continue regardless of the zeros')
...
#CHECK_VAR_ZEROS = args.CHECK_VAR_ZEROS
OUTDIR = args.OUTDIR
plot_out = args.plot
...
rfe_p = args.rfep
max_zero_checks=args.max_zero_checks
...
elif RANK_METHOD == 'randomForest' or SVM_TYPE=='randomForest':
...
if SVM_TYPE=='randomForest':
		...
		bs=Bootstrap(len(x[0]), n_iter=CV_N, random_state=0)
		for train_index, test_index in bs:
			print ("Train:",train_index,"Test",test_index)
			xs.append((train_index,test_index))
print "xs",xs
print len(xs)
'''

#####WHAT ABOUT RF first option and SVM classifier?
...
for n in range(CV_N):
	...
	#print CHECK_VAR_ZEROS
	#if True:#SVM_TYPE!='randomForest':
	flag=True
	seed=n
	counter=0
	max_zero_checks
	while flag:
		...
		for (i, (idx_tr, idx_ts)) in enumerate(idx):
			...
			if any(np.var(x_tr,axis=0)==0):
				prod=0
		if prod==0 and counter<max_zero_checks:	
			seed+=CV_N
		else:
			flag=False
	...
	for (i, (idx_tr, idx_ts)) in enumerate(idx):
		...
		for j, s in enumerate(FSTEPS):
			...
			if SVM_TYPE!='randomForest':
				...
				"""#print OUTDIR+"/classifier/randomForest"+str(i_classif)+".classifier"
				fout=open(OUTDIR+"/classifier/randomForest"+str(i_classif)+".classifier",'w')
				cPickle.dump([forest,v],fout,-1)
				i_classif+=1"""
				#print 
				
				#print j,s
				#print "x_fs_ts",x_ts_fs
...
pred_features=[SCALING, SVM_TYPE, opt_feats]
np.savetxt(OUTFILE + "_pred_features.log", np.array(pred_features), fmt='%s', delimiter='\t')
